...
class WPInstagram {
    ...
    public static function filter_tags() {
        $symbols = get_option(self::IMPORT_DIRECTIVES, '');

        $matches = array();
        preg_match_all('/#[A-Za-z0-9_]+/i', $symbols, $matches);
    ...
    protected static function perform_ajax_sync($admin = true)
    {
        ...
        $did_sync = false;
        $perform_api_call = get_transient( 'wp_instagram_api_call' );
        if ( empty( $perform_api_call ) ) {
